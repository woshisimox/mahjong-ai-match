...
export default function Home(){
  ...
  async function playOneHand(ps: PlayerState[], w: string[]){
    for(let turn=0; turn<256; turn++){
      ...
      const discardLine = `${seatLabel}-${ps[i].ai} 打出 ${tileLabel(out)}（理由：${reasonText}）`;
      appendLogs([seatLabel+' => '+apiLine, discardLine]);
      setPlayers([...ps]);
  ...
  return (<div className="max-w">
    ...
    <div className="card">
      <div className="flex items-center gap-3" style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
        <label className="small">最大轮次：<input type="number" step=500 min=500 step={500} min={500} className="w-24" value={intervalMs} onChange={e=>setIntervalMs(Math.max(500, Number(e.target.value) || 500))} />setMaxHands(Math.max(1,parseInt(e.target.value||'0',10)||1))} /></label>
