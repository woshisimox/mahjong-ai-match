  ...
  async function playOneHand(psInit: PlayerState[], wallInit: string[]){
    ...
    while(true){
      ...
      if(kakan){
        ...
        for(let r=1;r<ps.length;r++){
          ...
          if(canRon){
            ...
            ps = (rule==='sichuan_strict'? settlementRonStrict(ps, j, cur, wr) : settlementRon(ps, j, cur, wr));
      ...
      if(rSelf.win){
        ...
        ps = (rule==='sichuan_strict'? settlementTsumoStrict(ps, cur, wr) : settlementTsumo(ps, cur, wr));
        setPlayers(ps); break;
      }
...
function settlementTsumoStrict(ps: PlayerState[], who: number, wr: {yaku:string[]; fan:number}){
  const fan = Math.min(wr.fan, scCap);
  const pay = scBase * Math.pow(2, fan);
...
function settlementRonStrict(ps: PlayerState[], winner: number, loser: number, wr: {yaku:string[]; fan:number}){
  const fan = Math.min(wr.fan, scCap);
  const pay = scBase * Math.pow(2, fan);
  const next = ps.map(p=>({...p}));
