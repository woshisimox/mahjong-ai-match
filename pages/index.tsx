...
export default function Home(){
  ...
  function settlementTsumo(ps: PlayerState[], who: number, wr: {yaku:string[]; fan:number}){
    ...
    appendLogs([`结算：${label(who)} 自摸（${wr.yaku.join('+')}，共${base}番）`,
      ...ps.map((p,i)=>`  ${label(pi)}：${deltas[i]>0?'+':''}${deltas[i]}`)
    ]);
  ...
  async function startMatch(){
    ...
    let ps = dealHands(generateWall(), ['local','local','local','local']);
      ...
      <div className="grid" style={{gridTemplateColumns:'repeat(4,minmax(0,1fr))', gap:12}}>
        {players.map((p,pi)=>(
          <label key={'prov:'+pi} className="small">{label(pi)} 提供方
            <select className="w-full" value={p.ai}
              onChange={e=>{ const val=e.target.value; const np=[...players]; np[pi]={...np[pi], ai:val}; setPlayers(np); }}>
    ...
    <div className="card">
      ...
      <div className="mt-2">
        {players.map((p,pi) => (<div key={label(pi)} className="mb-3">
          <div className="font-semibold">{label(pi)}　<span className="text-xs" style={{opacity:.8}}>分：{p.score||0}</span></div>
          {showHands && (<>
