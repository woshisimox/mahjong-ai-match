...
export default function Home(){
      ...
      // 荣和优先
      {
        ...
        for(let k=1;k<ps.length;k++){
          ...
          if(checkWin([...ps[j].hand, out as string])){
            ...
            appendLogs([`${label(j)} 荣和 ${tileLabel(out!)}（${label(cur)} 放铳）`]);
            ps = settlementRon(ps, j, cur, wr);
  ...
  return (<div className="wrap">
    ...
    <style jsx>{`
      ...
      :global(body){ margin:0; background:#0b5a3c; color:#f2f5f3; }
      .wrap{ max-width:980px; margin:24px auto; padding:12px; }
      .card{ background:rgba(255,255,255,.06); backdrop-filter: blur(3px);
        border:1px solid rgba(255,255,255,.12); border-radius:12px; padding:12px 14px; margin-bottom:12px; }
      .row{ display:flex; gap:12px; align-items:center; flex-wrap:wrap; }
      .btn{ background:#0e7a47; color:#fff; padding:8px 12px; border-radius:8px; border:1px solid rgba(0,0,0,.3); }
      ...
      input, select{ background:rgba(255,255,255,.08); color:#f2f5f3; border:1px solid rgba(255,255,255,.18);
        border-radius:8px; padding:6px 8px; }
      .tiles{ display:flex; gap:6px; flex-wrap:wrap; }
      .tiles-wrap-14 .tile{ width:28px; }
      .tile{ display:inline-flex; width:32px; height:44px; border:1px solid #e6e6e6; border-radius:6px;
        align-items:center; justify-content:center; font-weight:700; background:#fffffe; color:#111;
        box-shadow: 0 2px 4px rgba(0,0,0,.35); }
      .tile.small{ width:24px; height:34px; font-weight:600; }
      .tile.w{ box-shadow: inset 0 0 0 2px rgba(200, 50, 50, .0), 0 2px 4px rgba(0,0,0,.35); }
      .tile.b{ box-shadow: inset 0 0 0 2px rgba(50, 120, 220, .0), 0 2px 4px rgba(0,0,0,.35); }
      .tile.t{ box-shadow: inset 0 0 0 2px rgba(20, 160, 90, .0), 0 2px 4px rgba(0,0,0,.35); }
      .tile.z{ box-shadow: inset 0 0 0 2px rgba(250, 250, 250, .0), 0 2px 4px rgba(0,0,0,.35); }
      .melds{ display:flex; gap:10px; flex-wrap:wrap; }
      .meld{ display:inline-flex; align-items:center; padding:3px 5px; border-radius:6px;
        border:1px dashed rgba(255,255,255,.18); background:rgba(255,255,255,.04); }
      .meld.chi{ background:rgba(46,204,113,.10); }
      .meld.pon{ background:rgba(52,152,219,.12); }
      .meld.ankan{ background:rgba(149,165,166,.18); }
      .meld.minkan{ background:rgba(241,196,15,.22); }
      .meld.kakan{ background:rgba(230,126,34,.20); }
      .text-xxs{ font-size:11px; }
      .log{ font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size:12px;
        max-height:260px; overflow:auto; background:rgba(0,0,0,.35); border:1px solid rgba(255,255,255,.12);
        border-radius:8px; padding:8px; color:#ecf3ee; }
    `}</style>
