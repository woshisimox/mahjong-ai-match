...
// === 新增：出牌后的“荣和”检查（传入 ps，避免作用域问题） ===
async function tryRon(psRef: PlayerState[], i:number, out:string){
  for(let k=1;k<psRef.length;k++){
    const j=(i+k)%psRef.length;
    const testHand=[...psRef[j].hand, out];
    const r2 = checkWin(testHand);
    if(r2.win){
      return { ron:true, j, r2 };
    }
  }
  return { ron:false as const };
}
 ...
 function startNewMatch() {
  ...
  async function playOneHand(ps: PlayerState[], w: string[]){
    for(let turn=0; turn<256; turn++){
      ...
      setPlayers([...ps]);
      // 荣和判定（有人胡则结束本局）
      const ronRes = await tryRon(ps, i, out);
      if(ronRes.ron){
        const { j, r2 } = ronRes as any;
        appendLogs([`${ps[j].ai} 荣和 ${tileLabel(out)}（${ps[i].ai} 放铳）：${r2.fan.join('+')} = ${r2.score}`]);
        const base=r2.score;
        ps = ps.map((p,idx)=> idx===j? {...p, score: p.score + base } : (idx===i ? {...p, score: p.score - base } : p));
        setPlayers(ps);
        break;
      }
