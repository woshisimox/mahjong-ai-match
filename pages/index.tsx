...
export default function Home(){
  ...
  function buildSnapshot(currentIndex: number){
    const N = 8; // 最近 N 次（每家上限）
    const me = players[currentIndex];
    // relative seats (clockwise): left = next player, across = opposite, right = previous
    const left = players[(currentIndex+1)%players.length]?.ai;
    const across = players[(currentIndex+2)%players.length]?.ai;
    const right = players[(currentIndex+players.length-1)%players.length]?.ai;

    // recents per player (last N)
    const recentsPerPlayer = players.map(p => ({
      ai: p.ai,
      recent: p.discards.slice(-N)
    }));

    // a merged approximate recent sequence (take from last tail round-robin)
    const mergedRecent: { ai: string, tile: string }[] = [];
    for(let k=1; k<=N; k++){
      for(let s=0; s<players.length; s++){
        const idx=(currentIndex+players.length-1-s)%players.length; // 从右手家开始向左
        const p = players[idx];
        const tile = p.discards[p.discards.length - k];
        if(tile) mergedRecent.push({ ai: p.ai, tile });
      }
    }

    // safety candidates: simple heuristic based on seen count in table discards
    const allDiscards = players.flatMap(p => p.discards);
    const seenCount: Record<string, number> = {};
    for(const t of allDiscards) seenCount[t]=(seenCount[t]||0)+1;
    const myCounts: Record<string, number> = {};
    for(const t of me.hand) myCounts[t]=(myCounts[t]||0)+1;
    const uniqMy = Array.from(new Set(me.hand));
    const riskTuple = (t:string) => {
      const suit=t[1]; const n=parseInt(t[0],10)||0;
      const seen = seenCount[t]||0;         // seen 越多越安全
      const honor = suit==='Z'?1:0;         // honors 稍高风险
      const middle = suit!=='Z' && n>=4 && n<=6 ? 1:0; // 中张稍高风险
      return [-seen, honor, middle]; // tuple sort ASC: 更小更安全
    };
    const safeCandidates = uniqMy
      .map(t=>({ tile:t, score: riskTuple(t) }))
      .sort((a,b)=>{
    ...
    return {
      me:{ ai: me.ai, discards: me.discards, score: me.score, handCount: me.hand.length },
      opponents: players.map((p,idx)=> idx!==currentIndex ? ({ ai:p.ai, discards:p.discards, score:p.score }) : null).filter(Boolean),
      table:{ wallCount: wall.length, handNo, maxHands, roundCount: handNo+1 },
      relative: { left, across, right },
      recents: { perPlayer: recentsPerPlayer, merged: mergedRecent },
      safety: { safeCandidates, seenCountSample: Object.fromEntries(Object.entries(seenCount).slice(0,20)) },
