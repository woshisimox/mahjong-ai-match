...
export default function Home(){
  ...
  // Structured snapshot: turn, lastDiscard, recentMoves, table, me/opponents
  const me = players[currentIndex];
  const opps = players
    .map((p,idx)=> idx!==currentIndex ? ({ ai:p.ai, discards:p.discards, score:p.score }) : null)
    .filter(Boolean) as any[];

  const lastIdx = (currentIndex - 1 + players.length) % players.length;
  const lastDiscardTile = players[lastIdx]?.discards?.slice(-1)[0] || null;

  const recentMoves = players.map(p => ({
    ai: p.ai,
    last3: (p.discards || []).slice(-3)
  }));

  return {
    turn: currentIndex,
    me: { ai: me.ai, hand: me.hand, discards: me.discards, score: me.score },
    opponents: opps,
    table: { wallCount: wall.length, handNo, maxHands },
    discardsAll: players.map(p => ({ ai: p.ai, discards: p.discards })),
    lastDiscard: { ai: players[lastIdx]?.ai, tile: lastDiscardTile },
    recentMoves
  };
}
  async function playOneHand(ps: PlayerState[], w: string[]){
    for(let turn=0; turn<256; turn++){
      ...
      ? `[API] ${ps[i].ai} ${apiMeta.usedApi ? `使用 ${apiMeta.provider} API` : '未调用外部API'}${apiMeta.detail ? ' - ' + apiMeta.detail : ''}${apiMeta.raw ? ` | raw: ${String(apiMeta.raw).slice(0,60)}` : ''}${apiMeta.got ? ` | got:${apiMeta.got}→${apiMeta.norm}` : ''}`
