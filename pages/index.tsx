...
export default function Home(){
  ...
  function buildSnapshot(currentIndex: number){
    ...
    const recentsPerPlayer = players.map(p => ({ ai: p.ai, recent: p.discards.slice(-N) }));
    const mergedRecent: { ai: string, tile: string }[] = [];
    for(let k=1; k<=N; k++){
      for(let s=0; s<players.length; s++){
        const idx=(currentIndex+players.length-1-s)%players.length;
        const p = players[idx];
    ...
    const riskTuple = (t:string) => {
      ...
      const seen = seenCount[t]||0;
      const honor = suit==='Z'?1:0;
      const middle = suit!=='Z' && n>=4 && n<=6 ? 1:0;
      return [-seen, honor, middle];
    };
    const safeCandidates = uniqMy.map(t=>({tile:t, score:riskTuple(t)})).sort((a,b)=>{
      const A=a.score, B=b.score;
      if(A[0]!==B[0]) return A[0]-B[0];
      if(A[1]!==B[1]) return A[1]-B[1];
      if(A[2]!==B[2]) return A[2]-B[2];
      return a.tile<b.tile?-1:a.tile>b.tile?1:0;
    }).map(x=>x.tile).slice(0,6);
  ...
  return (<div className="max-w">
    <h1 style={{fontSize:24,fontWeight:700,marginBottom:12}}>Mahjong AI Match v3.3.4c-fix</h1>
