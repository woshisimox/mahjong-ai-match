...
import {
  generateWall, generateWallEx, isFlower, dealHands, drawTile,
  checkWin, scoreWin, scoreWinSichuanStrict,
...
export default function Home(){
  ...
  async function playOneHand(psInit: PlayerState[], wallInit: string[]){
    ...
    while(true){
      ...
      ps[cur].hand.push(t);
      if(useFlowers && rule!=='sichuan'){
        while(isFlower(ps[cur].hand[ps[cur].hand.length-1])){
          const f = ps[cur].hand.pop()!; ps[cur].flowers = [...(ps[cur].flowers||[]), f];
          appendLogs([`${label(cur)} 补花：${tileLabel(f)}`]);
          const t2 = drawTile(w); if(!t2) break; ps[cur].hand.push(t2);
        }
      }
      ...
      if(kakan){
        ...
        for(let r=1;r<ps.length;r++){
          ...
          if(canRon){
            ...
            ps = (rule==='sichuan_strict'? settlementRonStrict(ps, j, cur, wr, (typeof scCap==='number'?scCap:13), (typeof scBase==='number'?scBase:1)) : settlementRon(ps, j, cur, wr));
        ...
        if(rule==='sichuan_strict'){
          const pay = scBase * Math.pow(2,1);
          const aliveIdx = ps.map((p,idx)=> idx!==cur && p.alive!==false ? idx : -1).filter(x=>x>=0);
          let gain=0; for(const i2 of aliveIdx){ ps[i2].score -= pay; gain += pay; }
          ps[cur].score += gain;
          appendLogs([`杠结算(严)：${label(cur)} 加杠 收入 ${gain}（每家 ${pay}）`]);
        }
        setPlayers([...ps]);
        // 加杠后岭上摸
        afterKanDraw = true;
      ...
      if(ankan){
        ...
        if(rule==='sichuan_strict'){
          const pay = scBase * Math.pow(2,2);
          const aliveIdx = ps.map((p,idx)=> idx!==cur && p.alive!==false ? idx : -1).filter(x=>x>=0);
          let gain=0; for(const i2 of aliveIdx){ ps[i2].score -= pay; gain += pay; }
          ps[cur].score += gain;
          appendLogs([`杠结算(严)：${label(cur)} 暗杠 收入 ${gain}（每家 ${pay}）`]);
        }
        setPlayers([...ps]);
        afterKanDraw = true;
        await new Promise(r=>setTimeout(r, intervalMs));
      ...
      // 自摸判定（岭上摸后自摸将包含岭上开花番）
      const rSelf = ((rule==='sichuan' || rule==='sichuan_strict') && ps[cur].que && ps[cur].hand.some(x=>x[1]===ps[cur].que))
        ? { win:false, yaku:[], fan:0 }
        : (rule==='sichuan_strict'
            ? scoreWinSichuanStrict(ps[cur].hand, ps[cur].melds, { tsumo:true, rinshan: wasRinshan, capFan: scCap })
            : scoreWin(ps[cur].hand, { tsumo:true, rinshan: wasRinshan })
          );
      if(rSelf.win){
        const wr = rSelf;
        appendLogs([`${label(cur)} 自摸：${wr.yaku.join('+')} = ${wr.fan}`]);
        ps = (rule==='sichuan_strict'? settlementTsumoStrict(ps, cur, wr, (typeof scCap==='number'?scCap:13), (typeof scBase==='number'?scBase:1)) : settlementTsumo(ps, cur, wr));
        setPlayers(ps); break;
      }
      ...
      // 明杠（来自弃牌）：优先于碰/吃
      {
        ...
        for(let k=1;k<ps.length;k++){
          ...
          if(canMinkan(ps[j].hand, out!)){
            ...
            if(rule==='sichuan_strict'){
              const pay = scBase * Math.pow(2,1);
              const aliveIdx = ps.map((p,idx)=> idx!==j && p.alive!==false ? idx : -1).filter(x=>x>=0);
              let gain=0; for(const i2 of aliveIdx){ ps[i2].score -= pay; gain += pay; }
              ps[j].score += gain;
              appendLogs([`杠结算(严)：${label(j)} 明杠 收入 ${gain}（每家 ${pay}）`]);
            }
            setPlayers([...ps]);
            // 明杠后由 j 岭上摸
            cur = j;
      ...
      if(chiPick && rule!=='sichuan' && rule!=='sichuan_strict'){
        // 从下家手里移除吃用的两张
    ...
    <div className="card">
      <div className="row">
        <label className="small">规则
          <select value={rule} onChange={e=>setRule(e.target.value as any)} disabled={matchActive||handRunning}>
        ...
        <label className="small"><input type="checkbox" checked={useFlowers} disabled={(rule==='sichuan'||rule==='sichuan_strict')||matchActive||handRunning} onChange={e=>setUseFlowers(e.target.checked)} /> 启用花牌</label>
      </div>
    </div>
...
function settlementTsumoStrict(ps: PlayerState[], who: number, wr: {yaku:string[]; fan:number}, cap:number, base:number){
  ...
  next[who].score += gain;
  appendLogs([`结算(严)：${label(who)} 自摸（${wr.yaku.join('+')}，${fan}番，底=${base} → 每家 ${pay}）`,
    ...next.map((p,i)=>`  ${label(i)}：${(i===who?'+':'')}${(i===who?gain:(i===who?0:-pay))}`)
  ]);
  return next;
}));
  let gain = 0;
  ...
  next[who].score += gain;
  appendLogs([`结算(严)：${label(who)} 自摸（${wr.yaku.join('+')}，${fan}番，底=${scBase} → 每家 ${pay}）`,
    ...next.map((p,i)=>`  ${label(i)}：${(i===who?'+':'')}${(i===who?gain:(i===who?0:-pay))}`)
  ]);
  return next;
}
function settlementRonStrict(ps: PlayerState[], winner: number, loser: number, wr: {yaku:string[]; fan:number}, cap:number, base:number){
  ...
  appendLogs([`结算(严)：${label(winner)} 荣和（${label(loser)} 放铳；${wr.yaku.join('+')}，${fan}番，底=${base} → 赔 ${pay}）`]);
  return next;
}));
  next[winner].score += pay;
  if(next[loser].alive!==false) next[loser].score -= pay;
