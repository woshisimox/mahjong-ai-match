...
export default function Home(){
  ...
  React.useEffect(()=>{
    try{
      const k = (typeof window!=='undefined' && sessionStorage.getItem('aiKeys')) as string | null; if(k) setKeys(JSON.parse(k));
      const s=sessionStorage.getItem('seatAIs'); if(s) setSeatAIs(JSON.parse(s));
    }catch{}
  },[]);
  React.useEffect(()=>{ try{ if (typeof window!=='undefined') { sessionStorage.setItem('aiKeys', JSON.stringify(keys||{})); } }catch{} }, [keys]);
  React.useEffect(()=>{ try{ sessionStorage.setItem('seatAIs', JSON.stringify(seatAIs||[])); }catch{} }, [seatAIs]);
