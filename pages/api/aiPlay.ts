...
export default async function handler(req:NextApiRequest, res:NextApiResponse){
  ...
  try{
    ...
    const local = ()=>{
      ...
      return { tile: best, reason: 'local' };
    };

    const ks:Keys = keys||{};
    if(ai==='chatgpt' && ks.chatgpt){ const r=await callOpenAI(ks.chatgpt, hand); return res.json(r); }
    if(ai==='kimi' && ks.kimi){ const r=await callMoonshot(ks.kimi, hand); return res.json(r); }
    if(ai==='grok' && ks.grok){ const r=await callGrok(ks.grok, hand); return res.json(r); }
    if(ai==='gemini' && ks.gemini){ const r=await callGemini(ks.gemini, hand); return res.json(r); }

    return res.json(local());
  }catch(e:any){
    return res.status(200).json({ tile: (req.body?.hand||[])[0], reason: 'fallback-error' });
  }
}
